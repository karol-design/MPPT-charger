...
/* Charging process and MPPT algorithm */
#define RUN_TIME 900000   // Main loop iteration time (~900 s = ~15 minutes)

class Solar_charger {
  public:
    Solar_charger();
    uint16_t get(uint8_t command);
    uint16_t setCharging(float Voltage_V);
    void ledBlink();
    void buzzer();
    void LM2576(uint8_t state);
    void report(uint8_t serialPort, bool Bulk);

  private:
    unsigned long readings_avr_mV, readings_sum;
    uint16_t _BAT_voltage, _PV_voltage, _PV_current, V_div_R1, V_div_R2;
    static uint8_t n_serial, n_bluetooth; 
    bool _Bulk;
};
